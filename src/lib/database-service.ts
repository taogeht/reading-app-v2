...
import { Recording } from '../api/recordings/index';
...
export class DatabaseService {
  ...
  static async getAllRecordings(): Promise<Recording[]> {
    const poolAvailable = await waitForPool();
    if (!poolAvailable) {
      console.warn('DatabaseService.getAllRecordings not available - pool not ready');
      return [];
    }

    try {
      const result = await pool.query(`
        SELECT id, student_id, assignment_id, attempt_number, audio_url, audio_filename,
               audio_size_bytes, audio_duration_seconds, transcript, feedback_data,
               accuracy_score, reading_pace, word_count, correct_words, status,
               processing_started_at, processing_completed_at, error_message,
               submitted_at, created_at, updated_at, is_archived, file_path
        FROM recordings 
        ORDER BY created_at DESC
      `);
  ...
  static async getRecordingsByAssignment(assignmentId: string): Promise<Recording[]> {
    const poolAvailable = await waitForPool();
    if (!poolAvailable) {
      console.warn('DatabaseService.getRecordingsByAssignment not available - pool not ready');
      return [];
    }
    ...
    try {
      ...
      console.error('Error getting recordings by assignment:', error);
      return [];
    }
  }

  static async getRecordingsByStudent(studentId: string): Promise<Recording[]> {
    const poolAvailable = await waitForPool();
    if (!poolAvailable) {
      console.warn('DatabaseService.getRecordingsByStudent not available - pool not ready');
      return [];
    }
  ...
  static async createRecording(recordingData: {
    ...
    try {
      const result = await pool.query(`
        INSERT INTO recordings (
          student_id, assignment_id, attempt_number, audio_url, audio_filename,
          audio_size_bytes, audio_duration_seconds, status, file_path,
          submitted_at, created_at, updated_at
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW(), NOW())
        RETURNING id, student_id, assignment_id, attempt_number, audio_url, audio_filename,
                 audio_size_bytes, audio_duration_seconds, transcript, feedback_data,
                 accuracy_score, reading_pace, word_count, correct_words, status,
                 processing_started_at, processing_completed_at, error_message,
                 submitted_at, created_at, updated_at, is_archived, file_path
      `, [
        recordingData.student_id,
        recordingData.assignment_id,
        recordingData.attempt_number,
        recordingData.audio_url,
        recordingData.audio_filename,
        recordingData.audio_size_bytes,
        recordingData.audio_duration_seconds,
  ...
  static async updateRecording(id: string, updates: {
    status?: 'uploaded' | 'processing' | 'completed' | 'failed';
    transcript?: string | null;
    feedback_data?: any | null;
    accuracy_score?: number | null;
  ...
  static async archiveRecording(id: string): Promise<boolean> {
    const poolAvailable = await waitForPool();
    if (!poolAvailable) {
      console.warn('DatabaseService.archiveRecording not available - pool not ready');
      return false;
  ...
  static async unarchiveRecording(id: string): Promise<boolean> {
    const poolAvailable = await waitForPool();
    if (!poolAvailable) {
      console.warn('DatabaseService.unarchiveRecording not available - pool not ready');
      return false;
