-- Secure Admin RLS Policies (FINAL VERSION)
-- This migration creates secure admin access without infinite recursion
-- FIXES: Properly handles all function dependencies including visual_passwords
...
-- ===================================================================
-- STEP 2: CREATE NEW SECURE ADMIN CHECK FUNCTIONS
-- ===================================================================
...
-- ===================================================================
-- STEP 3: CREATE SECURE RLS POLICIES FOR ALL TABLES
-- ===================================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'visual_passwords' AND schemaname = 'public') THEN
        EXECUTE 'CREATE POLICY "Admin can access all visual passwords"
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'class_sessions' AND schemaname = 'public') THEN
        EXECUTE 'CREATE POLICY "Admin can access all class sessions"
                 ON public.class_sessions FOR ALL
                 TO authenticated
                 USING (public.is_admin())';
                 
        EXECUTE 'CREATE POLICY "Teachers can access own class sessions"
                 ON public.class_sessions FOR ALL
                 TO authenticated
                 USING (
                     class_id IN (
        ...
        EXECUTE 'CREATE POLICY "Students can access their class sessions"
...
-- RECORDING SUBMISSIONS TABLE POLICIES (if table exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'recording_submissions' AND schemaname = 'public') THEN
        EXECUTE 'CREATE POLICY "Admin can access all recording submissions"
                 ON public.recording_submissions FOR ALL
                 TO authenticated
                 USING (public.is_admin())';
                 
        EXECUTE 'CREATE POLICY "Students can access own recording submissions"
                 ON public.recording_submissions FOR ALL
                 TO authenticated
                 USING (student_id = auth.uid())';
                 
        EXECUTE 'CREATE POLICY "Teachers can access recording submissions in their classes"
                 ON public.recording_submissions FOR SELECT
                 TO authenticated
                 USING (
                     assignment_id IN (
                         SELECT id FROM public.assignments WHERE teacher_id = auth.uid()
...
SELECT 'Secure admin RLS policies created successfully! All dependencies handled.' AS status;